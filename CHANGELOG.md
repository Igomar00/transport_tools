...
- new `penetration_span` option for `ambiguous_event_assignment_resolution` that resolves events falling into multiple superclusters by the surface-to-core depth range they traverse inside each supercluster (deepest minus shallowest buried point) rather than by their single deepest point, favouring genuine traversals over perpendicular crossings — without needing MD trajectories or AQUA-DUCT traces
- new `directionality` option for `ambiguous_event_assignment_resolution` that resolves multi-supercluster events by how closely the event's exit bearing matches each supercluster's overall direction (breaking ties within 10° by penetration span), rejecting superclusters the event merely crosses perpendicularly — also trajectory- and trace-free
- new trajectory-free `trace_matching`: matches each transport event's AQUA-DUCT per-frame trace against the actual per-snapshot CAVER tunnels of each supercluster, the trajectory-free counterpart of exact matching. Available both as an `ambiguous_event_assignment_resolution` option and as a standalone analysis via `perform_trace_matching_analysis` (writes per-event detail files for all assigned events)
- new `bundle_events_visualization` setting (default on): transport-event visualization is written as a single bundled CGO per (supercluster, scope) holding only the subsampled events with colours baked in, instead of one CGO file per event — cutting event-visualization files from O(events) to O(superclusters × scopes), and making `max_events_per_cluster4visualization` cap what is written, not just what is shown. The legacy per-event layout is kept when the setting is off or when `visualize_layered_events` is set
- scope configuration validation, setup, and network input loading to the stages a run actually executes: when resuming into a later stage (`start_from_stage`), inputs and checks needed only by earlier stages are no longer required
- new `caver_snapshot_stride` calculations setting (default `1`) that decouples CAVER snapshot sparsity from `snapshots_per_simulation`: when CAVER analyses only every Nth simulation frame (e.g. a 20000-frame trajectory thinned to 1000 snapshots → stride 20), event frames are mapped to the correct CAVER snapshot instead of assuming a dense 1:1 `frame + caver_traj_offset` correspondence. A new `SnapshotFrameMap` helper separates the three roles `snapshots_per_simulation` was overloaded with (analyzed-snapshot count, normalization denominator, and frame↔snapshot mapping), and the CAVER snapshot labelling (dense `1..N` vs frame-numbered) is auto-detected from the observed snapshot IDs. At the default stride `1` every formula reduces to the previous behaviour, so existing analyses are unaffected. The configured stride is validated against the AQUA-DUCT `Frames window` at stage 1 and against the parsed CAVER snapshot IDs at stage 2, failing fast on a wrong stride or on `snapshots_per_simulation` mistakenly set to a trajectory length; `caver_snapshot_stride` is part of the stage-2 tunnel-network shard fingerprint so changing it invalidates stale shards
- new `interpolate_missing_snapshots4matching` calculations setting (default `False`) for event-to-tunnel matching (both exact and trajectory-free trace matching under a sampling stride): by default an event frame that falls between analysed CAVER snapshots contributes no tunnel (exact time alignment, so the ligand is never matched against a tunnel from a different frame); when enabled, such a frame is instead matched against its nearest analysed snapshot. Event visualization always draws tunnels only for real analysed snapshots regardless of this setting
- new tunnel-profile pruning integrated into stage 2 (input processing) via the `prune_tunnels` calculations setting (default `False`), truncating CAVER tunnel tails that extend past the protein surface into empty solvent space: per CAVER cluster and MD, a consensus exit distance is derived from a joint-transition score (radius expansion × survival drop, `prune_tunnels_mode` `'first'`/`'peak'`), each tunnel reaching beyond that cut is classified as inflating into empty space and/or curved, and offensive tunnels are truncated in-place at a guarded position. Only relevant tunnels (`filters_passed`) take part — they define the consensus cut and the post-cut inflation reference and are the only ones truncated, so irrelevant stubs cannot pull the valid-range percentiles into the core region and distort the cut; clusters with fewer than two relevant tunnels are left untouched. The guarded cut never reaches the bottleneck (it keeps one sphere past `argmin(R)`, bracketing the true minimum, which CAVER locates between profile samples) and enforces a water-radius floor on the last kept sphere, leaving a tunnel untouched when no sphere past the guard meets it. Only sphere tails are removed; `length` and `curvature` are recomputed from the kept spheres in CAVER's own terms — the arc length of the sphere centerline from the first sphere, over the straight first-to-last distance, neither measured from the tunnel starting point whose gap to the first sphere CAVER excludes — so they reproduce CAVER's reported values exactly for an unmodified tunnel and truncated tunnels stay comparable with the untouched ones in their cluster. `cost`, `throughput` and `bottleneck_radius` cannot be derived from sphere data and keep their CAVER values (a logged limitation). The refreshed `length`, `curvature`, layer membership and relevant-tunnel filter verdicts propagate downstream so stages 3+ see the pruned geometry. Tuning knobs live in the advanced settings (`prune_tunnels_bin_size`, `prune_tunnels_survival_perc_low/high`, `prune_tunnels_core_fraction_low/high`, `prune_tunnels_min_joint_threshold`, `prune_tunnels_eff_thresh`, `prune_tunnels_slope_percentile`, `prune_tunnels_water_radius`) and are part of the stage-2 SLURM shard fingerprint

### Bug Fixes
- stop discarding whole tunnel clusters during stage-3 layering ("Cluster X of Y cannot be layered"): the adjustment of node layering for the global starting point derived a node's layer from the node average, while `split_points_to_layers()` derives it per point. The average of points spread over a spherical shell lies radially inside that shell, so wide nodes close to the starting point were relabeled one layer below every point they represent; `_validate_path()` then saw a two-layer jump in every path crossing such a node and, for a node shared by all tunnels of a cluster, left the cluster without a single path. The node's own points now decide its layer, the "do not empty the source layer" guard is evaluated at move time rather than on a pre-move snapshot, and a reassignment that leaves a cluster without any path is reverted instead of costing the cluster. On a 3000-simulation analysis this recovered all 639 rejected tunnel clusters, 99 of them among the top ten CAVER clusters of their simulation, while leaving the layered representation of unaffected clusters unchanged
- stop false-abandoning pending SLURM array shards as cancelled when `sacct` is disabled: `squeue` collapses pending array elements into a single entry, so completion is now tracked against an internally maintained shard list instead
- correct the number of transformation matrices reported in the stage-1 progress bar
